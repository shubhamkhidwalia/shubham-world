...
function dbGet(){ try{ const r=localStorage.getItem("sw-v11"); return r?JSON.parse(r):{}; }catch{ return {}; } }
function dbSet(d){ try{ localStorage.setItem("sw-v11",JSON.stringify(d)); }catch{} }

function score(item,idx){ const w={"★★★ Obsessed":3,"★★ Loved":2,"★ Liked":1}[item.rating]||0; return w*10+Math.max(0,100-idx*3); }
...
async function aiFill(name,catLabel,onStatus){
  ...
  const res = await fetch("https://api.anthropic.com/v1/messages",{
    ...
    body:JSON.stringify({
      ...
      max_tokens:900,
      system:"You are a pop-culture expert. Respond ONLY with a single valid JSON object — zero markdown, zero fences, zero prose. Must parse with JSON.parse().",
      messages:[{role:"user",content:`Details for: "${name}" (category: ${catLabel}). Return ONLY this JSON:\n{"description":"2–3 engaging sentences","genre":"1–3 word genre","year":"4-digit year or empty","fact":"surprising fact","tagline":"famous tagline max 8 words or empty","imageUrl":"real direct image URL ending .jpg .jpeg .png or .webp from Wikipedia/Wikimedia/TMDb/official","link":"best URL: Wikipedia, IMDb, Spotify, YouTube or official site"}`}]
    })
  });
  ...
  for(const c of candidates){ try{ const obj=JSON.parse(c); if(obj.description||obj.genre||obj.imageUrl) return obj; }catch{} }
  throw new Error("Couldn't parse response — fill in manually");
}

const CSS = `
${GFONTS}
*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
:root{--black:#000;--bg:#121212;--card:#181818;--hover:#282828;--line:#2a2a2a;--green:#1DB954;--green-h:#1ED760;--t1:#fff;--t2:#B3B3B3;--t3:#6A6A6A;--gold:#F5C518;--red:#E50914;}
...
.bg-wash-img{position:absolute;inset:0;background-size:cover;background-position:center;filter:blur(120px) brightness(.2) saturate(3);transform:scale(1.3);transition:background-image 1.2s ease;}
.bg-wash-overlay{position:absolute;inset:0;background:radial-gradient(ellipse 80% 60% at 50% 0%,transparent,rgba(0,0,0,.75) 100%);}
...
.nav.stuck{background:rgba(10,10,10,.97);border-bottom:1px solid var(--line);}
...
.logo-dot{width:10px;height:10px;border-radius:50%;background:var(--green);animation:gpulse 2.4s ease-in-out infinite;cursor:pointer;flex-shrink:0;transition:transform .15s,box-shadow .15s;}
.logo-dot.tapped{animation:none;transform:scale(1.5);box-shadow:0 0 0 10px rgba(29,185,84,.12);}
...
.owner-pill{padding:4px 14px;border-radius:100px;background:var(--green);color:#000;font-size:11px;font-weight:800;letter-spacing:.06em;cursor:pointer;transition:background .2s,transform .15s;}
.owner-pill:hover{background:var(--green-h);transform:scale(1.04);}
...
.nav-search-input:focus{background:rgba(255,255,255,.12);border-color:rgba(255,255,255,.25);width:250px;}
...
.nav-search-clear{position:absolute;right:10px;background:none;border:none;color:var(--t3);font-size:14px;cursor:pointer;line-height:1;}
...
.sdot-cat{width:6px;height:6px;border-radius:50%;flex-shrink:0;}
...
.spot-grad{position:absolute;inset:0;background:linear-gradient(to right,rgba(0,0,0,.92),rgba(0,0,0,.5) 50%,rgba(0,0,0,.04)),linear-gradient(to top,rgba(0,0,0,.95),transparent 55%),linear-gradient(to bottom,rgba(0,0,0,.45),transparent 25%);z-index:1;}
.spot-body{position:relative;z-index:2;height:100%;display:flex;align-items:center;padding:80px 60px 60px;gap:60px;}
.spot-left{flex:1;max-width:640px;}
...
.spot-title{font-family:'Bebas Neue',sans-serif;font-size:clamp(52px,8.5vw,106px);line-height:.88;letter-spacing:.02em;margin-bottom:18px;text-shadow:0 4px 40px rgba(0,0,0,.8);animation:sIn .48s cubic-bezier(.22,.61,.36,1) both;}
@keyframes sIn{from{opacity:0;transform:translateY(22px)}to{opacity:1;transform:none}}
...
.spot-tagline{font-size:14px;font-style:italic;color:var(--green);border-left:2px solid var(--green);padding-left:14px;margin-bottom:18px;line-height:1.55;animation:sIn .48s .08s cubic-bezier(.22,.61,.36,1) both;}
.spot-desc{font-size:15px;color:var(--t2);line-height:1.65;max-width:520px;margin-bottom:32px;animation:sIn .48s .14s cubic-bezier(.22,.61,.36,1) both;}
.spot-btns{display:flex;gap:12px;flex-wrap:wrap;animation:sIn .48s .19s cubic-bezier(.22,.61,.36,1) both;}
...
.spot-poster{width:clamp(160px,17vw,250px);height:clamp(240px,25.5vw,375px);border-radius:16px;overflow:hidden;background:var(--card);display:flex;align-items:center;justify-content:center;font-size:64px;box-shadow:0 40px 100px rgba(0,0,0,.9),0 0 0 1px rgba(255,255,255,.08);animation:pIn .65s .15s cubic-bezier(.22,.61,.36,1) both;position:relative;}
...
.stats-bar{background:rgba(255,255,255,.025);border-top:1px solid var(--line);border-bottom:1px solid var(--line);padding:14px 36px;display:flex;align-items:center;gap:32px;overflow-x:auto;position:relative;z-index:2;}
...
.nrow-track{display:flex;gap:0;padding:4px 28px 28px;overflow-x:auto;overflow-y:visible;scroll-behavior:smooth;scrollbar-width:none;-webkit-overflow-scrolling:touch;align-items:flex-end;}
...
.narr{position:absolute;top:50%;transform:translateY(-65%);width:36px;height:72px;border-radius:4px;z-index:5;background:rgba(10,10,10,.95);border:1px solid rgba(255,255,255,.12);color:#fff;font-size:22px;display:flex;align-items:center;justify-content:center;opacity:0;pointer-events:none;transition:all .2s;cursor:pointer;}
...
.rcard-num{font-family:'Bebas Neue',sans-serif;font-size:108px;line-height:.85;letter-spacing:-3px;color:transparent;-webkit-text-stroke:2.5px rgba(155,155,155,.38);user-select:none;flex-shrink:0;margin-right:-20px;position:relative;z-index:0;}
.rcard-wrap:first-child .rcard-num{-webkit-text-stroke:2.5px rgba(210,210,210,.55);}
.rcard-wrap:nth-child(2) .rcard-num{-webkit-text-stroke:2.5px rgba(190,190,190,.47);}
.rcard-wrap:nth-child(3) .rcard-num{-webkit-text-stroke:2.5px rgba(170,170,170,.42);}
...
.det-box{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);z-index:951;width:min(860px,95vw);max-height:90vh;background:var(--card);border:1px solid rgba(255,255,255,.1);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;animation:bPop .3s cubic-bezier(.34,1.56,.64,1);}
...
.det-fact{background:var(--hover);border-radius:8px;padding:14px 16px;border-left:3px solid var(--green);font-size:13px;color:var(--t2);line-height:1.65;}
...
.modal-bg{position:fixed;inset:0;z-index:980;background:rgba(0,0,0,.90);display:flex;align-items:center;justify-content:center;padding:20px;animation:fadeIn .16s ease;will-change:opacity;}
.modal-box{background:var(--card);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:36px;width:100%;max-width:500px;max-height:92vh;overflow-y:auto;animation:slideUp .22s cubic-bezier(.22,.61,.36,1);will-change:transform,opacity;}
...
.toast{position:fixed;bottom:28px;right:28px;z-index:9999;background:var(--hover);border:1px solid rgba(255,255,255,.12);border-radius:100px;padding:12px 20px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:10px;animation:tpop .3s cubic-bezier(.34,1.56,.64,1);box-shadow:0 16px 40px rgba(0,0,0,.7);}
...
function LockModal({onClose,onUnlock}){
  ...
  const go=()=>{ if(pw===OWNER_PASS){onUnlock();onClose();}else{setErr(true);setPw("");setTimeout(()=>setErr(false),1800);} };
  return(
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="modal-box lm">
        ...
        <input className="fi" type="password" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Password…" autoFocus/>
        {err&&<div className="lm-err">WRONG PASSWORD</div>}
        <div className="mftr" style={{marginTop:20}}><button className="mc" onClick={onClose}>Back</button><button className="ms" onClick={go}>Unlock →</button></div>
...
function EditModal({cat,edit,onClose,onSave}){
  ...
  useEffect(()=>{ const fn=e=>e.key==="Escape"&&!busy&&onClose(); window.addEventListener("keydown",fn); return()=>window.removeEventListener("keydown",fn); },[busy]);
  const fill=async()=>{
    ...
    try{
      ...
      setAiSt({type:"ok",msg:"✓ All fields filled! Check the image preview below."});
    }catch(e){ setAiSt({type:"err",msg:`⚠ ${e.message}`}); }
    setBusy(false);
  };
  const save=()=>{ if(!name.trim()) return; onSave({name:name.trim(),link:link.trim(),image:image.trim(),desc:desc.trim(),fact:fact.trim(),tagline:tagline.trim(),genre:genre.trim(),year:year.trim(),rating}); };
  return(
    <div className="modal-bg" onClick={e=>e.target===e.currentTarget&&!busy&&onClose()}>
      <div className="modal-box">
        <div className="mhdr"><div className="mtitle">{edit?`Edit · ${edit.name.slice(0,22)}`:`Add · ${cat.label}`}</div><button className="mclose" onClick={onClose} disabled={busy}>×</button></div>
        <div className="fg"><label className="fl">Name *</label><input className="fi" value={name} onChange={e=>setName(e.target.value)} placeholder={`Your favourite ${cat.label.toLowerCase()}…`}/></div>
        <div className="ai-bar">
          <div className="ai-icon">✨</div>
          <div className="ai-text"><div className="ai-title">AI Auto-Fill</div><div className="ai-sub">Fills all fields from Claude's knowledge</div></div>
          {busy?<div className="ai-spin"><i/><i/><i/></div>:<button className="ai-btn" onClick={fill} disabled={!name.trim()}>{edit?"Re-Fill ✨":"Auto Fill ✨"}</button>}
        </div>
        {aiSt&&<div className={`ai-status ${aiSt.type}`}>{aiSt.type==="loading"&&<div className="ai-spin" style={{marginRight:4}}><i/><i/><i/></div>}{aiSt.msg}</div>}
        <div className="two">
        ...
        <div className="fg"><label className="fl">Link</label><input className="fi" value={link} onChange={e=>setLink(e.target.value)} placeholder="https://…"/>
        ...
        <div className="fg"><label className="fl">Cover Image URL</label><input className="fi" value={image} onChange={e=>setImage(e.target.value)} placeholder="Direct .jpg / .png / .webp URL…"/><ImgPreview src={image}/></div>
        <div className="fg"><label className="fl">Your Rating</label><div className="rrow">{RATINGS.map(r=><div key={r} className={`ropt ${rating===r?"on":""}`} onClick={()=>setRating(rating===r?"":r)}>{r}</div>)}</div></div>
        <div className="mftr"><button className="mc" onClick={onClose} disabled={busy}>Cancel</button><button className="ms" onClick={save} disabled={!name.trim()||busy}>{edit?"Save Changes":"Add →"}</button></div>
      </div>
    </div>
  );
}
    ...
    <>
      ...
      <div className="det-box">
        <div className="det-hero">
          ...
          <div className="det-actions">
            {isOwner&&<>
              ...
              <button className="det-x" style={{background:"rgba(229,9,20,.2)",color:"#ff5555"}} onClick={()=>{if(window.confirm("Remove this item?")) onDelete(item.id);}}>🗑</button>
            </>}
        ...
        <div className="det-body">
          ...
          <div className="det-imdb-row"><div className="imdb-box">IMDb</div><div className="imdb-stars">{stars}</div><div className="imdb-lbl">Shubham's Pick</div></div>
        ...
        <div className="det-footer">
          ...
          {isOwner&&<><button className="det-btn det-btn-g" onClick={()=>{onClose();onEdit(item);}}>✏ Edit</button><button className="det-btn det-btn-d" onClick={()=>{if(window.confirm("Remove?")) onDelete(item.id);}}>🗑 Remove</button></>}
    ...
    <div className="nrow">
      <div className="nrow-hdr">
        ...
        <div className="nrow-right">
          {items.length>1&&<><button className={`nrow-sort ${sort==="rating"?"on":""}`} onClick={()=>setSort(s=>s==="rating"?"default":"rating")}>Rating</button><button className={`nrow-sort ${sort==="year"?"on":""}`} onClick={()=>setSort(s=>s==="year"?"default":"year")}>Year</button></>}
      ...
      {items.length===0&&!isOwner?<div className="nrow-empty">Nothing here yet.</div>:(
        <div className="nrow-wrap">
          ...
          <div className="nrow-track" ref={ref}>
            ...
            {isOwner&&<div className={`nc-add ${SHAPE[cat.id]||"tall"}`} onClick={onAdd} style={{marginLeft:sorted.length?10:0}}><div className="nc-add-plus">+</div><div className="nc-add-lbl">Add</div></div>}
          </div>
...
function Spotlight({tops,isOwner,onOpen,onAdd,onBgChange}){
  ...
  useEffect(()=>{ if(tops[idx]) onBgChange?.(tops[idx]); },[idx,tops]);
  if(!tops.length) return(
    <div className="spot-empty">
      <div style={{fontSize:72,opacity:.1}}>🌌</div>
      <div className="spot-empty-h">Your Vault Awaits</div>
      <div className="spot-empty-s">{isOwner?"Tap the dot 5× to unlock, then add stuff.":"Nothing saved yet."}</div>
    </div>
  );
  ...
  return(
    <div className="spot">
      ...
      <div className="spot-bottom">
        <div className="spot-dots">{tops.map((_,i)=><div key={i} className={`sdot ${i===idx?"act":"sm"}`} onClick={()=>goTo(i)}>{i===idx&&<div className="sdot-fill" key={aKey}/>}</div>)}</div>
          ...
          {results.length===0?<div className="search-empty">No results for "{q}"</div>:results.map(({item,cat})=>(
            <div key={item.id} className="search-item" onClick={()=>{setDetail({item,cat});setOpen(false);setQ("");}}>
              ...
              <div className="search-info">
                ...
                <div className="search-meta"><div className="sdot-cat" style={{background:cat.color}}/><span style={{color:cat.color,fontSize:10,fontWeight:700}}>{cat.label}</span>{item.year&&<span>{item.year}</span>}{item.rating&&<span>{item.rating.split(" ")[0]}</span>}</div>
              </div>
            </div>
          ))}
        </div>
...
export default function App(){
  ...
  const handleDot=()=>{
    ...
    if(tapCount.current>=5){ tapCount.current=0; if(isOwner){setIsOwner(false);showT("Vault locked","🔒");}else setShowLock(true); return; }
  ...
  const bgImgUrl=bgItem?.item?.image||tops[0]?.item?.image||null;
  const bgGradients=tops.slice(0,5).map((t,i)=>{
    const pos=["18% 22%","78% 18%","50% 65%","88% 78%","12% 80%"][i];
    return `radial-gradient(ellipse 40% 38% at ${pos},${t.cat.color}14,transparent 68%)`;
  }).join(",");
  ...
  return(
    <>
      ...
      <div className="bg-wash">
        {bgImgUrl&&<div className="bg-wash-img" style={{backgroundImage:`url(${bgImgUrl})`}}/>}
        {tops.length>0&&<div style={{position:"absolute",inset:0,background:bgGradients,pointerEvents:"none"}}/>}
        <div className="bg-wash-overlay"/>
      </div>
      ...
      <Spotlight tops={tops} isOwner={isOwner} onOpen={openDet} onAdd={()=>openAdd(CATS[0])} onBgChange={t=>setBgItem(t)}/>
      {total>0&&(
      ...
      <div className="content">
        ...
        <div className="site-footer">{total} favourites · {linked} links<br/><span style={{opacity:.45}}>Made with 🖤 by Shubham</span></div>
      </div>
      {detail&&<Detail item={detail.item} cat={detail.cat} isOwner={isOwner} onClose={()=>setDetail(null)} onEdit={item=>{setDetail(null);openEdit(item,detail.cat);}} onDelete={handleDelete}/>}
      {showLock&&<LockModal onClose={()=>setShowLock(false)} onUnlock={()=>{setIsOwner(true);showT("Swagat hai Shubham! 🎉","🔓");}}/>}
